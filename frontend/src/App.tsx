...
function AuthPage({ mode, onModeChange, onBack, onAuthenticated }: { mode: AuthMode; onModeChange: (mode: AuthMode) => void; onBack: () => void; onAuthenticated: () => void }) {
      ...
      <div className="relative z-10 w-full max-w-5xl overflow-hidden rounded-[30px] border border-white/80 bg-white/75 shadow-[0_28px_80px_rgba(65,75,140,0.16)] backdrop-blur-xl md:grid md:grid-cols-[0.9fr_1.1fr]">
        ...
        <section className="p-7 sm:p-12">
          <div className="mx-auto max-w-sm">
            ...
            <div key={mode} className="auth-form-swap min-h-[340px]"><form className="mt-7" onSubmit={(event) => { event.preventDefault(); onAuthenticated() }}><label className="text-xs font-bold text-[#575b70]">Email address<input type="email" placeholder="you@example.com" className="mt-2 w-full rounded-xl border border-[#e1e3ed] px-4 py-3 text-sm outline-none transition focus:border-[#4b5ce1] focus:ring-4 focus:ring-[#4b5ce1]/10" /></label>{!isSignIn && <label className="mt-4 block text-xs font-bold text-[#575b70]">Your name<input type="text" placeholder="Alex Morgan" className="mt-2 w-full rounded-xl border border-[#e1e3ed] px-4 py-3 text-sm outline-none transition focus:border-[#4b5ce1] focus:ring-4 focus:ring-[#4b5ce1]/10" /></label>}<label className="mt-4 block text-xs font-bold text-[#575b70]">Password<input type="password" placeholder="••••••••" className="mt-2 w-full rounded-xl border border-[#e1e3ed] px-4 py-3 text-sm outline-none transition focus:border-[#4b5ce1] focus:ring-4 focus:ring-[#4b5ce1]/10" /></label><button className="mt-6 w-full rounded-xl bg-[#4b5ce1] py-3.5 text-sm font-semibold text-white transition hover:bg-[#3f50d4]">{isSignIn ? 'Sign in' : 'Create account'} <span className="ml-1">→</span></button></form></div>
...
function Dashboard({ profile }: { profile: Profile }) {
  ...
  return <main className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8 lg:px-12 lg:py-12"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-[#85899c]">Good morning, {profile.name.split(' ')[0] || 'there'}</p><h1 className="mt-2 text-3xl font-bold tracking-[-0.06em] text-[#20243b] sm:text-4xl">Tailor your resume</h1><p className="mt-3 max-w-xl text-sm leading-6 text-[#777b8d]">Start with a job posting and we’ll prepare your resume for the opportunity ahead.</p></div><span className="hidden rounded-full bg-[#e9f8f1] px-3 py-2 text-xs font-bold text-[#299567] sm:inline-flex">Your profile is ready</span></div>
    ...
    {submitted && <section className="mt-5 rounded-2xl border border-[#d9e9e2] bg-[#f5fcf8] p-5 sm:p-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="mt-0.5 grid h-8 w-8 place-items-center rounded-full bg-[#39a779] text-white"><CheckIcon /></span><div><p className="font-bold text-[#286a4e]">Job details saved</p><p className="mt-1 text-sm text-[#5f8976]">Your posting is ready for resume generation.</p></div></div><button onClick={generateResume} disabled={generating || generated} className="rounded-xl bg-[#286a4e] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#205a42] disabled:cursor-not-allowed disabled:opacity-70">{generating ? 'Creating your tailored resume…' : generated ? 'Resume created' : 'Create my tailored resume'} {generated && <span className="ml-1">✓</span>}</button></div>{generating && <div className="mt-5 h-2 overflow-hidden rounded-full bg-[#d9eee3]"><div className="h-full w-1/2 animate-pulse rounded-full bg-[#39a779]" /></div>}{generated && <div className="mt-5 rounded-xl border border-[#d9e9e2] bg-white px-4 py-3 text-sm font-semibold text-[#286a4e]">Your tailored resume is ready for the next step.</div>}</section>}
...
function FileDropzone({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  const inputRef = useRef<HTMLInputElement>(null)
  const acceptFile = (file?: File) => { if (file) onChange(file.name) }
  return <div><p className="mb-2 text-xs font-bold text-[#575b70]">{label}</p>{value ? <div className="flex items-center justify-between rounded-xl border border-[#d9e9e2] bg-[#f5fcf8] px-4 py-3"><div className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-white text-[#39a779]"><FileIcon /></span><span className="truncate text-sm font-semibold text-[#286a4e]">{value}</span></div><button type="button" onClick={() => onChange('')} className="text-xs font-bold text-[#7d9c8d] hover:text-[#286a4e]">Replace</button></div> : <button type="button" onClick={() => inputRef.current?.click()} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); acceptFile(event.dataTransfer.files[0]) }} className="flex w-full flex-col items-center justify-center rounded-xl border border-dashed border-[#cfd3e2] bg-[#fafbfe] px-5 py-7 text-center transition hover:border-[#4b5ce1] hover:bg-[#f7f8ff]"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e9ebff] text-[#4b5ce1]"><UploadIcon /></span><span className="mt-3 text-sm font-bold text-[#4b5ce1]">Drop a file here or browse</span><span className="mt-1 text-xs text-[#9296a7]">PDF, DOC, or DOCX</span><input ref={inputRef} type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={event => acceptFile(event.target.files?.[0])} /></button>}</div>
}
...
function SettingsPage({ profile, onSave }: { profile: Profile; onSave: (profile: Profile) => void }) {
  ...
  const save = (event: React.FormEvent) => { event.preventDefault(); onSave(draft); setToast('Account settings saved') }
  return <main className="mx-auto w-full max-w-4xl px-5 py-8 sm:px-8 lg:px-12 lg:py-12"><div><p className="text-sm font-semibold text-[#85899c]">Your account</p><h1 className="mt-2 text-3xl font-bold tracking-[-0.06em] text-[#20243b]">Account Settings</h1><p className="mt-3 text-sm leading-6 text-[#777b8d]">Keep your profile and career documents ready for every application.</p></div><form onSubmit={save} className="mt-9 rounded-2xl border border-[#e4e6f0] bg-white p-5 shadow-[0_18px_50px_rgba(65,75,140,0.06)] sm:p-8"><div className="grid gap-5 sm:grid-cols-2"><label className="text-xs font-bold text-[#575b70]">Name<input value={draft.name} onChange={event => update('name', event.target.value)} className="mt-2 w-full rounded-xl border border-[#e1e3ed] px-4 py-3 text-sm outline-none focus:border-[#4b5ce1] focus:ring-4 focus:ring-[#4b5ce1]/10" /></label><label className="text-xs font-bold text-[#575b70]">Email address<input type="email" value={draft.email} onChange={event => update('email', event.target.value)} className="mt-2 w-full rounded-xl border border-[#e1e3ed] px-4 py-3 text-sm outline-none focus:border-[#4b5ce1] focus:ring-4 focus:ring-[#4b5ce1]/10" /></label><label className="text-xs font-bold text-[#575b70]">GitHub repository<input value={draft.github} onChange={event => update('github', event.target.value)} placeholder="https://github.com/you/project" className="mt-2 w-full rounded-xl border border-[#e1e3ed] px-4 py-3 text-sm outline-none placeholder:text-[#b2b5c2] focus:border-[#4b5ce1] focus:ring-4 focus:ring-[#4b5ce1]/10" /></label><label className="text-xs font-bold text-[#575b70]">Portfolio<input value={draft.portfolio} onChange={event => update('portfolio', event.target.value)} placeholder="https://yourportfolio.com" className="mt-2 w-full rounded-xl border border-[#e1e3ed] px-4 py-3 text-sm outline-none placeholder:text-[#b2b5c2] focus:border-[#4b5ce1] focus:ring-4 focus:ring-[#4b5ce1]/10" /></label></div><div className="my-8 border-t border-[#eceef5]" /><div><h2 className="text-base font-bold text-[#282c43]">Career documents</h2><p className="mt-1 text-sm text-[#85899c]">Upload the latest materials you want ResuME to use.</p></div><div className="mt-6 space-y-5"><FileDropzone label="Recent resume" value={draft.resume} onChange={value => update('resume', value)} /><FileDropzone label="Background document" value={draft.background} onChange={value => update('background', value)} /><FileDropzone label="Cover letter (optional)" value={draft.coverLetter} onChange={value => update('coverLetter', value)} /></div><div className="mt-8 flex justify-end border-t border-[#eceef5] pt-6"><button className="rounded-xl bg-[#4b5ce1] px-5 py-3 text-sm font-semibold text-white shadow-[0_9px_20px_rgba(75,92,225,0.18)] transition hover:bg-[#3f50d4]">Save changes</button></div></form>{toast && <Toast message={toast} onDismiss={() => setToast('')} />}</main>
}

function AuthenticatedApp({ onSignOut }: { onSignOut: () => void }) {
  const [page, setPage] = useState<AppPage>('dashboard')
  const [profile, setProfile] = useState<Profile>(() => { try { return { ...defaultProfile, ...JSON.parse(localStorage.getItem('resume-profile') || '{}') } } catch { return defaultProfile } })
  const saveProfile = (nextProfile: Profile) => { setProfile(nextProfile); localStorage.setItem('resume-profile', JSON.stringify(nextProfile)) }
...
function App() {
  const [view, setView] = useState<AppView>(() => localStorage.getItem('resume-authenticated') === 'true' ? 'app' : 'landing')
  ...
  const authenticate = () => {
    localStorage.setItem('resume-authenticated', 'true')
    setView('app')
  }

  const signOut = () => {
    localStorage.removeItem('resume-authenticated')
    setView('landing')
  }
