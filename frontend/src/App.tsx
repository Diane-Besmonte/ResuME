...
function SettingsPage({ profile, onSave, onConnect, onDisconnect }: { profile: Profile; onSave: (profile: Profile, files: Partial<Record<'resume' | 'background' | 'coverLetter', File>>) => Promise<Profile>; onConnect: () => Promise<void>; onDisconnect: () => Promise<void> }) {
...
function AuthenticatedApp({ onSignOut }: { onSignOut: () => void }) {
  const [page, setPage] = useState<AppPage>(() => localStorage.getItem(OPENAI_KEY_STORAGE) ? 'dashboard' : 'provider')
  const [showProviderPrompt, setShowProviderPrompt] = useState(() => !localStorage.getItem(OPENAI_KEY_STORAGE))
  ...
  return <div className="min-h-screen bg-[#f7f8fc] text-[#171a2c] lg:flex"><Sidebar page={page} onPageChange={setPage} onSignOut={onSignOut} profile={profile} /><div className="min-w-0 flex-1 page-transition" key={page}>{page === 'dashboard' ? <Dashboard profile={profile} onConfigure={() => setPage('provider')} /> : page === 'provider' ? <ProviderPage /> : <SettingsPage profile={profile} onSave={saveProfile} onConnect={connectGithub} onDisconnect={disconnectGithub} />}</div>{showProviderPrompt && <div className="provider-prompt fixed inset-0 z-50 grid place-items-center bg-[#20243b]/30 px-5 backdrop-blur-sm"><div role="dialog" aria-modal="true" aria-labelledby="provider-prompt-title" className="w-full max-w-md rounded-2xl bg-white p-7 shadow-[0_24px_70px_rgba(32,36,59,0.22)]"><div className="grid h-11 w-11 place-items-center rounded-xl bg-[#e9ebff] text-[#4b5ce1]"><Sparkle /></div><h2 id="provider-prompt-title" className="mt-5 text-xl font-bold text-[#282c43]">Set up your AI provider</h2><p className="mt-2 text-sm leading-6 text-[#777b8d]">Add your OpenAI API key before creating a tailored resume. It stays in this browser and is only used during generation.</p><button onClick={() => { setShowProviderPrompt(false); setPage('provider') }} className="mt-6 w-full rounded-xl bg-[#4b5ce1] py-3 text-sm font-semibold text-white transition hover:bg-[#3f50d4]">Go to AI provider <span className="ml-1">→</span></button></div></div>}</div>
}
