...
function AuthenticatedApp({ onSignOut }: { onSignOut: () => void }) {
  ...
  return <div className="min-h-screen bg-[#f7f8fc] text-[#171a2c] lg:flex"><Sidebar page={page} onPageChange={setPage} onSignOut={onSignOut} profile={profile} /><div className="min-w-0 flex-1">{page === 'dashboard' ? <Dashboard profile={profile} onConfigure={() => setPage('settings')} /> : <SettingsPage profile={profile} onSave={saveProfile} onConnect={connectGithub} />}</div></div>
}
